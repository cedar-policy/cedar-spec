    ...
    match e
    {
      case And(e1, e2) =>
        ...
        calc == {
          ...
          CE.interpret(core.And(env.replaceUnknownInExpr(e1), env.replaceUnknownInExpr(e2)));
        }
            ...
            case _ =>
              match PE.interpret(e2) {
                ...
                case Err(_) =>
                  calc == {
                    peRes;
                    Ok(Residual.And(r1, PE.makeErrorValue()));
                  }
                  calc == {
                    env.interpret(peRes.value, s);
                    env.interpret(Residual.And(r1, PE.makeErrorValue()), s);
                  }
                  if env.interpret(r1, s).Err? {
                    eval.InterpretResidualAndErr(env, r1, PE.makeErrorValue(), s);
                    assert env.interpret(Residual.And(r1, PE.makeErrorValue()), s).Err?;
                    assert CE.interpret(env.replaceUnknownInExpr(e1)).Err? by {
                      assert util.relaxedEq(env.interpret(r1, s), CE.interpret(env.replaceUnknownInExpr(e1)));
                    }
