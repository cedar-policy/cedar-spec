...
module validation.thm.strict_soundness {
  ...
  datatype StrictProof = StrictProof(
    ...
    {
      assert P_TC.ensureIntType(e, effs).Ok? by {
        assert S_TC.ensureIntType(e, effs).Ok?;
        StrictTypecheckingIsStrict(e, effs);
      }
    }
    ...
    {
      assert P_TC.ensureIntType(e1, effs).Ok? by {
        assert S_TC.ensureIntType(e1, effs).Ok?;
        StrictTypecheckingIsStrict(e1, effs);
      }
      assert P_TC.ensureIntType(e2, effs).Ok? by {
        assert S_TC.ensureIntType(e2, effs).Ok?;
        StrictTypecheckingIsStrict(e2, effs);
      }
    }
