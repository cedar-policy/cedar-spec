...
// Alternate version of the engine that reports arithmetic overflow.
//
// To minimize code duplication, we call subroutines from the main engine
// whenever we can. Unfortunately, much of the code has to remain duplicated
// because we want it to call the implementation from this engine if it finds a
// subexpression that needs an overflow check. If you diff this file against
...
module def.engine_with_overflow {
  ...
  datatype Evaluator = Evaluator(request: Request, store: EntityStore) {
    const evaluatorWithoutOverflow := engine.Evaluator(request, store)
    ...
    function interpret(expr: Expr): Result<Value> {
      match expr {
        ...
        case GetAttr(e, a) =>
          var v :- interpret(e);
          var r :- evaluatorWithoutOverflow.expectRecordDerefEntity(v, false);
          if a in r.Keys then Ok(r[a]) else Err(AttrDoesNotExist)
        case HasAttr(e, a) =>
          var v :- interpret(e);
          var r :- evaluatorWithoutOverflow.expectRecordDerefEntity(v, true);
        ...
        case Call(name, es) =>
          ...
          evaluatorWithoutOverflow.applyExtFun(name, args)
        case _ => evaluatorWithoutOverflow.interpret(expr)
      }
    }
    ...
    function applyUnaryOp(uop: UnaryOp, x: Value): Result<Value> {
      match uop {
        ...
        case _ => evaluatorWithoutOverflow.applyUnaryOp(uop, x)
      }
    }
    ...
    function applyBinaryOp(bop: BinaryOp, x: Value, y: Value): Result<Value> {
      match bop {
        ...
        case _ => evaluatorWithoutOverflow.applyBinaryOp(bop, x, y)
      }
    }
  }
}
