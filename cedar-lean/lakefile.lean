...
-- Prototype: runtime support for parsers generated by LeanParserGenerator.
-- These files are copied verbatim from the LeanParserGenerator repo and keep
-- their original `LeanParserGenerator.*` module names, so they live at the
-- project root (like `Cedar/`) and the generated parser/lexer imports resolve
-- unchanged. Only the non-dependent runtime + lexer engine is copied (no
-- verified/proof modules); `globs` builds the whole copied subtree.
lean_lib LeanParserGenerator where
  globs := #[Glob.submodules `LeanParserGenerator]
  defaultFacets := #[LeanLib.staticFacet]
