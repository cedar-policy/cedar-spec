...
@[export loadProtobufPolicySet] unsafe def loadProtobufPolicySet (req: ByteArray) : Except String Policies :=
  ((@Message.interpret? Proto.PolicySet) req |>.mapError (s!"failed to parse input: {·}")) >>= (·.toPolicies)
...
/--
  `policies`: pre-parsed policy set
  `req`: binary protobuf for a `RequestEnv`

  returns JSON encoded of the term generated by `verifyAlwaysAllows`
--/
@[export assertsOfCheckAlwaysAllows] unsafe def assertsOfCheckAlwaysAllows (schema : Cedar.Validation.Schema) (policies : Policies) (req : ByteArray) : String :=
  runFfiM do
    let req ← (@Message.interpret? Proto.RequestEnv) req |>.mapError (s!"failed to parse input: {·}")
    let env ← match schema.environment? req.principal req.resource req.action with
      | none => .error s!"failed to get environment from requestEnv (PrincipalType: {req.principal}, ActionName: {req.action}, ResourceType: {req.resource})"
      | some env => .ok env
    let εnv := SymEnv.ofTypeEnv env
...
--/
@[export assertsOfCheckAlwaysDenies] unsafe def assertsOfCheckAlwaysDenies (schema : Cedar.Validation.Schema) (policies : Policies) (req : ByteArray) : String :=
  runFfiM do
    let req ← (@Message.interpret? Proto.RequestEnv) req |>.mapError (s!"failed to parse input: {·}")
    let env ← match schema.environment? req.principal req.resource req.action with
      | none => .error s!"failed to get environment from requestEnv (PrincipalType: {req.principal}, ActionName: {req.action}, ResourceType: {req.resource})"
...
/--
  ...
  2.) .ok {data := SMTLib-Script, duration := encode_time} where SMTLib-Script is a
      string containing the SMTLib script encoding the verification query
-/
@[export smtLibOfCheckAlwaysAllows] unsafe def smtLibOfCheckAlwaysAllows (schema : Cedar.Validation.Schema) (policies : Policies) (req : ByteArray) : String :=
  runFfiM do
    let req ← (@Message.interpret? Proto.RequestEnv) req |>.mapError (s!"failed to parse input: {·}")
    let env ← match schema.environment? req.principal req.resource req.action with
      | none => .error s!"failed to get environment from requestEnv (PrincipalType: {req.principal}, ActionName: {req.action}, ResourceType: {req.resource})"
...
/--
  ...
  2.) .ok {data := SMTLib-Script, duration := encode_time} where SMTLib-Script is a
      string containing the SMTLib script encoding the verification query
--/
@[export smtLibOfCheckAlwaysDenies] unsafe def smtLibOfCheckAlwaysDenies (schema : Cedar.Validation.Schema) (policies : Policies) (req : ByteArray) : String :=
  runFfiM do
    let req ← (@Message.interpret? Proto.RequestEnv) req |>.mapError (s!"failed to parse input: {·}")
    let env ← match schema.environment? req.principal req.resource req.action with
      | none => .error s!"failed to get environment from requestEnv (PrincipalType: {req.principal}, ActionName: {req.action}, ResourceType: {req.resource})"
