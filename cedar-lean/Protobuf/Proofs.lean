...
theorem foldl_iterator_progress {α β : Type} {f: BParsec α} {g: β → α → β} {remaining: Nat} {init: β} {result: β} (H1: remaining > 0) (H: (foldl f g remaining init) it1 = .success it2 result) : it2.pos > it1.pos := by
  ...
  induction remaining using Nat.strongInductionOn generalizing f g init result it1 it2
  next ni IH =>
    intro (H1: ni > 0)
...
theorem utf8DecodeChar.sizeGt0 (it1 it2: ByteArray.Iterator) (pos: Nat) (c: Char) (H: utf8DecodeChar pos it1 = .success it2 ⟨c, n⟩) : n > 0 := by
