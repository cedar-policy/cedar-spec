...
def Request.asPartialRequest (req : Request) : PartialRequest :=
  ...
  , context   := .some ∘ Map.mk $ req.context.toList.attach₂.map λ kv => (kv.val.fst, .present kv.val.snd.asPartialValue) }

open Cedar.TPE

def EntityData.asPartial (data : EntityData) : PartialEntityData :=
  { attrs := (.some ∘ Map.mk $ data.attrs.toList.attach₂.map λ kv => (kv.val.fst, .present kv.val.snd.asPartialValue))
  , ancestors := (.some data.ancestors)
  , tags := (.some ∘ Map.mk $ data.tags.toList.attach₂.map λ kv => (kv.val.fst, .present kv.val.snd.asPartialValue))}
