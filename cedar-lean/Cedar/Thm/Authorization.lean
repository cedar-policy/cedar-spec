...
-/
theorem determiningPolicies_of (request: Request) (entities: Entities) (policies: Policies) (decision : Decision) :
  let auth := (isAuthorized request entities policies)
  auth.decision = decision →
  auth.determiningPolicies = satisfiedPolicies (if decision = .allow then .permit else .forbid) policies request entities
:= by
  intro auth
  simp [auth]
  unfold isAuthorized
  ...
  intro h
  split <;> rename_i hd <;> simp only [hd, ↓reduceIte, and_self] at h <;> simp [← h, h₁, h₂]
...
theorem unchanged_determining_when_add_policy_and_decision_unchanged (request: Request) (entities: Entities) (policies: Policies) (policy₀ : Policy) :
  ...
  simp only [auth, auth']
  rw [determiningPolicies_of request entities policies auth.decision rfl,
      determiningPolicies_of request entities (policy₀ :: policies) auth.decision (h₀.symm)]
  apply subset_lemma
