...
:= by
  ...
  cases h₃ : typeOf x₂ c env (l == Level.infinite) <;> simp [h₃] at h₁
  simp [typeOfBinaryApp, typeOfEq, ok, err, beq_iff_eq, List.empty_eq] at h₁
  rename_i tc₁ tc₂
  split at h₁
  case h_1 p₁ p₂ =>
    ...
    case _ =>
      ...
      case _ =>
        ...
        simp only [EmptyCollection.emptyCollection, and_self, Except.ok.injEq, Prod.mk.injEq,
          and_true, false_and, or_false, h₄, reduceCtorEq]
  case _ h₄  =>
    replace ⟨h₄, h₅⟩ := h₄
    constructor
    case _ =>
    ...
    case _ =>
      ...
      case _ =>
        ...
        simp only [Except.ok.injEq, Prod.mk.injEq, and_true, List.empty_eq, false_and, true_and,
          false_or, h₄, reduceCtorEq]
        simp [EmptyCollection.emptyCollection, List.instEmptyCollection, reduceCtorEq] at h₅
        assumption
...
theorem type_of_mem_is_soundₛ {x₁ x₂ : Expr} {c₁ c₁' c₂' : Capabilities} {env : Environment} {request : Request} {entities : Entities} {ety₁ ety₂ : EntityType} {l l₁ l₂ : Level} {ty : CedarType}
  ...
  case some =>
    ...
    case some =>
      ...
      case true =>
        replace ⟨euid', h₁₂⟩ := h₁₂
        simp [typeOfInₛ, typeOfInₛ.type, err, reduceCtorEq] at h₈
        split at h₈ <;> try assumption
        simp [actionUID?, entityUID?, hac, entityUIDS?_of_lits, reduceCtorEq]  at h₈
        split at h₈
        case _ =>
          simp [ok] at h₈
          apply h₉
...
theorem type_of_hasTag_inversion {x₁ x₂ : Expr} {c₁ c₂ : Capabilities} {env : Environment} {ty : CedarType} {l : Level}
  (h₁ : typeOf (Expr.binaryApp .hasTag x₁ x₂) c₁ env (l == .infinite) = .ok (ty, c₂)) :
  ∃ ety l' c₁' c₂',
    typeOf x₁ c₁ env (l == .infinite) = .ok (.entity ety l', c₁') ∧
    typeOf x₂ c₁ env (l == .infinite) = .ok (.string, c₂') ∧
    typeOfHasTag ety l' x₁ x₂ c₁ env  = .ok (ty, c₂)
:= by
  simp only [typeOf] at h₁
  cases h₂ : typeOf x₁ c₁ env (l == .infinite) <;> simp only [h₂, Except.bind_ok, Except.bind_err, reduceCtorEq] at h₁
  cases h₃ : typeOf x₂ c₁ env (l == .infinite) <;> simp only [h₃, Except.bind_ok, Except.bind_err, reduceCtorEq] at h₁
  rename_i tyc₁ tyc₂
  cases tyc₁
  cases tyc₂
...
theorem type_of_hasTag_is_sound {x₁ x₂ : Expr} {c₁ c₂ : Capabilities} {env : Environment} {ty : CedarType} {request : Request} {entities : Entities} {l : Level}
  (h₁ : CapabilitiesInvariant c₁ request entities)
  (h₂ : RequestAndEntitiesMatchEnvironment env request entities)
  (h₃ : typeOf (Expr.binaryApp .hasTag x₁ x₂) c₁ env (l == Level.infinite) = Except.ok (ty, c₂))
  (ih₁ : TypeOfIsSound x₁)
  (ih₂ : TypeOfIsSound x₂) :
...
:= by
  replace ⟨ety, l', c₁', c₂', h₄, h₅, h₃⟩ := type_of_hasTag_inversion h₃
  ...
  cases has_tag  : typeOfHasTag.mtype uid.ty x₁ x₂ c₁ env
    <;> simp [has_tag] at h₃
  split at h₃ <;> rename_i level_eq <;> simp [ok,err] at h₃
  rename_i tuple
  have ⟨ty', caps⟩ := tuple
  have ⟨ty_eq, caps_eq⟩ := h₃
  simp at ty_eq
  subst ty_eq
  simp at caps_eq
  subst caps_eq
  clear h₃
  unfold typeOfHasTag.mtype at has_tag
  split at has_tag
    <;> simp [ok,err] at has_tag
  case h_1 heq =>
    replace ⟨h₃, h₆⟩ := has_tag
    subst h₃ h₆
  ...
  case h_2 =>
    split at has_tag <;> simp only [Except.ok.injEq, Prod.mk.injEq] at has_tag <;>
    replace ⟨h₃, h₆⟩ := has_tag <;>
    subst h₃ h₆
    case isTrue hin =>
      simp only [hempty, implies_true, reduceCtorEq, false_or, exists_eq_left', true_and]
  ...
  case h_3 heq =>
    split at has_tag <;> simp only [Except.ok.injEq, Prod.mk.injEq, reduceCtorEq] at has_tag
    rename_i hact
    replace ⟨h₃, h₆⟩ := has_tag
    subst h₃ h₆
    simp only [hempty, implies_true, reduceCtorEq, false_or, exists_eq_left', true_and]
...
theorem type_of_getTag_inversion {x₁ x₂ : Expr} {c₁ c₂ : Capabilities} {env : Environment} {ty : CedarType} {l : Level}
  (h₁ : typeOf (Expr.binaryApp .getTag x₁ x₂) c₁ env (l == .infinite) = .ok (ty, c₂)) :
  c₂ = [] ∧
  ∃ ety l' c₁' c₂',
    typeOf x₁ c₁ env (l == .infinite) = .ok (.entity ety l', c₁') ∧
    typeOf x₂ c₁ env (l == .infinite) = .ok (.string, c₂') ∧
...
:= by
  ...
  cases get_tag : typeOfGetTag.mtype ety x₁ x₂ c₁ env
    <;> simp [get_tag] at h₁
  ...
  replace ⟨h₁, h₁'⟩ := h₁
  subst h₁ h₁'
  rename_i tuple gt_zero
  have ⟨end_type, end_caps⟩ := tuple
  unfold typeOfGetTag.mtype at get_tag
  split at get_tag <;> try simp [err] at get_tag
  split at get_tag <;> simp [ok,err] at get_tag
  rename_i a b
  simp [Except.ok.injEq, Prod.mk.injEq, CedarType.entity.injEq, true_and, and_true,
    exists_and_left, exists_and_right, exists_eq', exists_eq_left',  and_self, get_tag, a, b]

theorem type_of_getTag_is_sound {x₁ x₂ : Expr} {c₁ c₂ : Capabilities} {env : Environment} {ty : CedarType} {request : Request} {entities : Entities} {l : Level}
  (h₁ : CapabilitiesInvariant c₁ request entities)
  (h₂ : RequestAndEntitiesMatchEnvironment env request entities)
  (h₃ : typeOf (Expr.binaryApp .getTag x₁ x₂) c₁ env (l == .infinite) = Except.ok (ty, c₂))
  (ih₁ : TypeOfIsSound x₁)
  (ih₂ : TypeOfIsSound x₂) :
...
:= by
  replace ⟨hc, ety, l', c₁', c₂', h₃, h₄, h₅, h₆⟩ := type_of_getTag_inversion h₃
  subst hc
  replace ⟨_, v₁, ih₁, hty₁⟩ := ih₁ h₁ h₂ h₃
  replace ⟨_, v₂, ih₂, hty₂⟩ := ih₂ h₁ h₂ h₄
  simp only [EvaluatesTo] at *
