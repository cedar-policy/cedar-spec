...
/--
Given compiled policies `cpₛs` and some `asserts`, calls the SMT solver (if
...
environment such that evaluating `cpₛs` in `env` violates the property verified
by `asserts`.

The `asserts` are expected to be well-formed with respect to the `εnv` which `cpₛs` where compiled for, according to
`Cedar.SymCC.Term.WellFormed`.
(Caller guarantees that all of the `cpₛs` were compiled for the same `εnv`.)
...
(post-typecheck) policies that were compiled to `cpₛs`, and the meaning of `t` is
a function of the meaning of `xs`. This ensures that findings generated by
`solve` are sound and complete.
-/
def satAssertsOpt? (cpₛs : List CompiledPolicyₛ) (asserts : Asserts) : SolverM (Option Env) := do
  match cpₛs with
  | [] => throw (IO.userError s!"satAssertsOpt? called with empty list of policies")
  | cpₛ :: _ =>
  match ← checkSatAsserts asserts cpₛ.εnv with
  | .none   => pure none
  | .some I =>
    match extractOpt? cpₛs I with
...
def impliesOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [.policies ps₁, .policies ps₂] (verifyImpliesOpt ps₁ ps₂)
...
def alwaysAllowsOpt? (ps : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [.policies ps] (verifyAlwaysAllowsOpt ps)
...
def alwaysDeniesOpt? (ps : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [.policies ps] (verifyAlwaysDeniesOpt ps)
...
def equivalentOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [.policies ps₁, .policies ps₂] (verifyEquivalentOpt ps₁ ps₂)
...
def disjointOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [.policies ps₁, .policies ps₂] (verifyDisjointOpt ps₁ ps₂)
