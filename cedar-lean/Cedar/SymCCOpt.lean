...
/--
Given compiled policies `cps` and some `asserts`, calls the SMT solver (if
...
environment such that evaluating `cps` in `env` violates the property verified
by `asserts`.

The `asserts` are expected to be well-formed with respect to the `εnv` which `cps` where compiled for, according to
`Cedar.SymCC.Term.WellFormed`.
(Caller guarantees that all of the `cps` were compiled for the same `εnv`.)
...
(post-typecheck) policies that were compiled to `cps`, and the meaning of `t` is
a function of the meaning of `xs`. This ensures that findings generated by
`solve` are sound and complete.
-/
def satAssertsOpt? (cps : List CompiledPolicies) (asserts : Asserts) : SolverM (Option Env) := do
  match cps with
  ...
  | .none   => pure none
  | .some I =>
    match CompiledPolicies.extractOpt? cps I with
    | .some env => pure (some env)
    | .none     => throw (IO.userError s!"Extraction failed.")

/--
Alternate version of `satAssertsOpt?` which accepts a list of `CompiledPolicy`
instead of `CompiledPolicies`

Caller guarantees that all of the `CompiledPolicy`s were compiled for the same `εnv`.
-/
def satAssertsOpt?' (cps : List CompiledPolicy) (asserts : Asserts) : SolverM (Option Env) := do
  match cps with
  | [] => throw (IO.userError s!"satAssertsOpt?' called with empty list of policies")
  | { εnv, .. } :: _ =>
    match ← checkSatAsserts asserts εnv with
    | .none   => pure none
    | .some I =>
      match CompiledPolicy.extractOpt? cps I with
      | .some env => pure (some env)
      | .none     => throw (IO.userError s!"Extraction failed.")
...
def neverErrorsOpt? (p : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p] (verifyNeverErrorsOpt p)
...
def alwaysMatchesOpt? (p : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p] (verifyAlwaysMatchesOpt p)
...
def neverMatchesOpt? (p : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p] (verifyNeverMatchesOpt p)
...
def matchesEquivalentOpt? (p₁ p₂ : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p₁, p₂] (verifyMatchesEquivalentOpt p₁ p₂)
...
def matchesImpliesOpt? (p₁ p₂ : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p₁, p₂] (verifyMatchesImpliesOpt p₁ p₂)
...
def matchesDisjointOpt? (p₁ p₂ : CompiledPolicy) : SolverM (Option Env) :=
  satAssertsOpt?' [p₁, p₂] (verifyMatchesDisjointOpt p₁ p₂)
...
def impliesOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [ps₁, ps₂] (verifyImpliesOpt ps₁ ps₂)
...
def alwaysAllowsOpt? (ps : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [ps] (verifyAlwaysAllowsOpt ps)
...
def alwaysDeniesOpt? (ps : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [ps] (verifyAlwaysDeniesOpt ps)
...
def equivalentOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [ps₁, ps₂] (verifyEquivalentOpt ps₁ ps₂)
...
def disjointOpt? (ps₁ ps₂ : CompiledPolicies) : SolverM (Option Env) :=
  satAssertsOpt? [ps₁, ps₂] (verifyDisjointOpt ps₁ ps₂)
