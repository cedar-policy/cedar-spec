use cedar_policy_core::{ast, entities, extensions::Extensions, parser::{parse_policy_or_template, parse_policyset, Loc}};
...
fn output_dir() -> PathBuf {
    std::env::var("OUTPUT_DIR").map(PathBuf::from)
...
fn encode_schema(path: impl AsRef<Path>, s: &str) {
    let (schema, warnings) = cedar_policy_validator::ValidatorSchema::from_cedarschema_str(s, &Extensions::all_available()).map_err(|e| format!("{:?}", miette::Report::new(e))).unwrap();
...
fn main() {
    ...
    encode_expr("nested_set.protodata", "[ [1, 2], [3, principal.foo], false ]");
    encode_expr("emptyrecord.protodata", "{}");
    encode_expr("record.protodata", "{ ham: 3, eggs: 7 }");
    encode_expr("nested_record.protodata", r#"{ ham: { a: 0, b: false }, eggs: ["this is", "a set"] }"#);
    encode_expr("principal_in_resource_owners.protodata", "principal in resource.owners");
    encode_expr("has_and_get.protodata", r#"principal has department && principal.department == "Sales""#);
    encode_expr("has_and_get_tags.protodata", r#"principal.hasTag("department") && principal.getTag("department") == "Sales""#);
    encode_expr("not_and_neg.protodata", "!principal.foo || -(principal.bar) != 3");
    encode_expr("plus_and_minus_and_times.protodata", "32 + context.count - 7 * 4");
    encode_expr("contains.protodata", r#"[2,3,User::"alice"].contains(context.foo)"#);
    encode_expr("like.protodata", r#"context.a.b.c.d like "a*b**cd""#);
    encode_expr("is.protodata", r#"App::Widget::"123" is App::Widget && resource.widget is App::Widget in principal.widgets"#);
    encode_expr("complicated.protodata", "(if context.foo < 3 then principal.foo else principal.bar).a.b.contains(context.abc)");
    encode_expr("ip.protodata", r#"ip("192.168.0.1").isInRange(ip("192.0.0.0/8"))"#);
    encode_expr("decimal.protodata", r#"decimal("3.14").lessThan(decimal("3.1416"))"#);

    encode_policy_as_template("rbac.protodata", r#"permit(principal == User::"a b c", action, resource is App::Widget);"#);
    encode_policy_as_template("abac.protodata", r#"permit(principal, action, resource) when { principal == resource.owner } unless { resource.sensitive };"#);
    ...
    policyset.link(
        ...
        HashMap::from_iter([(ast::SlotId::principal(), ast::EntityUID::with_eid_and_type("User", "alice").unwrap())]),
    ).unwrap();
    encode_policyset("policyset.protodata", &policyset);

    encode_request("request.protodata", &ast::Request::new(
        (ast::EntityUID::with_eid_and_type("User", "alice").unwrap(), None),
        (ast::EntityUID::with_eid_and_type("Action", "access").unwrap(), Some(Loc::new(2..5, "source code".into()))),
        (ast::EntityUID::with_eid_and_type("Folder", "data").unwrap(), None),
        ast::Context::from_pairs([("foo".into(), ast::RestrictedExpr::val(true))], Extensions::all_available()).unwrap(),
        None::<&ast::RequestSchemaAllPass>,
        Extensions::all_available(),
    ).unwrap());

    encode_entity("entity.protodata", &ast::Entity::new(
        ast::EntityUID::from_components(ast::EntityType::from_normalized_str("A::B").unwrap(), ast::Eid::new("C"), None),
        [
    ...
    encode_entities("entities.protodata", &entities::Entities::from_entities(
        [
    ...
    let primitive_bool = validator_types::Type::Primitive { primitive_type: validator_types::Primitive::Bool };
    let primitive_long = validator_types::Type::Primitive { primitive_type: validator_types::Primitive::Long };
    let primitive_string = validator_types::Type::Primitive { primitive_type: validator_types::Primitive::String };
    ...
    encode_val_type("type_set_of_string.protodata", &validator_types::Type::Set { element_type: Some(Box::new(primitive_string.clone())) });
    encode_val_type("type_ip.protodata", &validator_types::Type::ExtensionType { name: ast::Name::parse_unqualified_name("ipaddr").unwrap() });
    encode_val_type("type_record.protodata", &validator_types::Type::EntityOrRecord(validator_types::EntityRecordKind::Record {
        attrs: validator_types::Attributes {
            attrs: BTreeMap::from_iter([
                ("ham".into(), validator_types::AttributeType::required_attribute(primitive_string.clone())),
                ("eggs".into(), validator_types::AttributeType::optional_attribute(primitive_long.clone())),
            ]),
        },
        open_attributes: validator_types::OpenTag::ClosedAttributes,
    }));

    encode_schema("schema.protodata", r#"
        entity A;
        entity B in A;
        entity C {
            a: Bool,
