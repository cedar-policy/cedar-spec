...
-/
@[inline]
private def findInMapValues [BEq α] [Hashable α] (m : List (α × (Std.HashSet α))) (k₁ : α) : Std.HashSet α :=
  let setOfSets := m.map λ (k₂,v) => if v.contains k₁ then (Std.HashSet.emptyWithCapacity 1).insert k₂ else Std.HashSet.emptyWithCapacity
  setOfSets.foldl Std.HashSet.union Std.HashSet.emptyWithCapacity

@[inline]
private def descendantsToAncestors [BEq α] [Hashable α] (descendants : List (α × (Std.HashSet α))) : Std.HashMap α (Std.HashSet α) :=
  Std.HashMap.ofList (descendants.map λ (k,_) => (k, findInMapValues descendants k))
...
def toSchema (schema : Schema) : Except String Validation.Schema := do
  ...
  let descendantMap := ets.map λ decl => (decl.name.toName, Std.HashSet.ofList $ decl.descendants.toList.map Spec.Proto.Name.toName)
  ...
  let ets ← ets.mapM λ decl => do
    ...
    let ese : EntitySchemaEntry ←
      if decl.enums.isEmpty then .ok $ .standard {
        ancestors := Data.Set.make (ancestorMap.getD name (Std.HashSet.emptyWithCapacity)).toList
  ...
  let descendantMap := acts.map λ decl => (decl.name, Std.HashSet.ofList decl.descendants.toList)
  let ancestorMap := descendantsToAncestors descendantMap
  let acts ← acts.mapM λ decl => do
    .ok (decl.name, {
      ...
      ancestors := Data.Set.make (ancestorMap.getD decl.name (Std.HashSet.emptyWithCapacity)).toList
