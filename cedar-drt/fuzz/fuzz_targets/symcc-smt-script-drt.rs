...
fn compile_rust_policy<'a>(
    ...
    Ok(compile_always_allows(&well_typed_policies, &sym_env)?)
}
...
fn smtlib_of_check_asserts(rust_asserts: &WellFormedAsserts<'_>) -> anyhow::Result<String> {
    ...
    RUNTIME.block_on(async {
        match solver.check_sat(rust_asserts).await {
            Ok(_) | Err(cedar_policy_symcc::err::Error::SolverUnknown) => Ok(()),
            Err(e) => Err(e),
        }
    })?;
    ...
    if let Ok(schema) = Schema::try_from(input.schema) {
        for req_env in schema.request_envs() {
            if let Ok(sym_env) = SymEnv::new(&schema, &req_env) {
                ...
                if let Ok(rust_asserts) =
                    compile_rust_policy(&sym_env, &policyset, &req_env, &schema)
                {
