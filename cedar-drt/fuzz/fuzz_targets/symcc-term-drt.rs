    ...
    if let Ok(schema) = Schema::try_from(input.schema) {
        ...
        for req_env in schema.request_envs() {
            ...
            if let Ok(compiled_policies) = CompiledPolicySet::compile(&policyset, &req_env, &schema)
            {
                ...
                // of `asserts_of_check_always_allows` because there are
                // discrepancies between the policies generated by Lean and Rust
                // validators. And the enforcer is sensitive to such
                // discrepancies.
                match lean_ffi.asserts_of_check_always_allows_on_original(
