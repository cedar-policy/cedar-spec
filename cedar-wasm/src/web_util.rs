 ...
 */
use crate::web_analysis::{AnalyzePolicyFindings, PerSigFindings, VacuityResult};
use crate::web_cli_enums::{ContextArg, RequestArgsEnum};
use crate::web_err::{ContentType, EntityType, ExecError, RequestElement};
use cedar_policy::{
    Context, Entities, EntityId, EntityTypeName, EntityUid, Expression, Request, RequestEnv, Schema,
};
use cedar_policy_core::ast::{Annotations, Policy, PolicyID, PolicySet};
...
/// Auxiliary function that parses a Policy using cedar_policy_core parser
pub fn parse_policy(fname: &PathBuf) -> Result<Policy, ExecError> {
    match read_to_string(fname) {
        Ok(policy_text) => {
            match cedar_policy_core::parser::parse_policy(None, &policy_text) {
                Ok(policy) => Ok(rename_from_id_annotation_policy(policy.into())),
...
fn rename_from_id_annotation_policy(policy: Policy) -> Policy {
    use cedar_policy_core::ast::{AnyId, PolicyID};
    use std::sync::Arc;
    use std::collections::BTreeMap;

    let id_str = AnyId::from_str("id").unwrap();
    match policy.annotation(&id_str) {
        None => policy,
        Some(anno) => {
            let anno_str = anno.as_ref();
            let id = PolicyID::from_string(anno_str); 
            // Collect annotations into a new BTreeMap
            let mut annos_map = BTreeMap::new();
            for (key, value) in policy.annotations() {
                annos_map.insert(key.clone(), value.clone());
            }
            
            // Create Annotations from the BTreeMap
            let annotations = Annotations::from(annos_map);
            
            // Create a new policy with the new ID
            // Use from_when_clause_annos to preserve annotations
            return Policy::from_when_clause_annos(
                policy.effect(),
                Arc::new(policy.condition().clone()),
                id,
                policy.loc().cloned(),
                Arc::new(annotations),
            );
...
/// Auxiliary function that parses a PolicySet using cedar_policy_core parser
pub fn parse_policyset(fname: &PathBuf) -> Result<PolicySet, ExecError> {
    match read_to_string(fname) {
        Ok(policyset_text) => {
            match cedar_policy_core::parser::parse_policyset(&policyset_text) {
                Ok(policyset) => Ok(policyset),
                Err(parse_err) => Err(ExecError::ParseError {
...
// Add these functions to web_util.rs
pub fn parse_policyset_from_string(content: &str) -> Result<cedar_policy_core::ast::PolicySet, crate::web_err::ExecError> {
    match cedar_policy_core::parser::parse_policyset(content) {
        Ok(policyset) => Ok(policyset),
        Err(parse_err) => Err(crate::web_err::ExecError::ParseError {
            content_type: crate::web_err::ContentType::PolicySet,
            file_name: PathBuf::from("in-memory-policy"),
            error: Box::new(parse_err),
        }),
    }
}
...
fn rename_from_id_annotation_policyset(ps: PolicySet) -> miette::Result<PolicySet> {
    use cedar_policy_core::ast::{AnyId, PolicyID, Annotations};
    use std::sync::Arc;
    use std::collections::BTreeMap;
    use miette::Context;
    ...
    // Process templates
    for template in ps.templates() {
        let id_str = AnyId::from_str("id").unwrap();
        let template_with_new_id = match template.annotation(&id_str) {
            None => template.clone(),
            Some(anno) => {
                let anno_str = anno.as_ref();
                let id = PolicyID::from_string(anno_str);
                // Use the new_id method to create a new template with the new ID
                template.new_id(id)
            }
        };
        
        new_ps.add_template(template_with_new_id)
    ...
    for policy in ps.policies() {
        let id_str = AnyId::from_str("id").unwrap();
        let policy_with_new_id = match policy.annotation(&id_str) {
            None => policy.clone(),
            Some(anno) => {
                let anno_str = anno.as_ref();
                let id = PolicyID::from_string(anno_str);
...
pub fn parse_schema_from_string(content: &str) -> Result<cedar_policy::Schema, crate::web_err::ExecError> {
    match cedar_policy::Schema::from_json_str(content) {
        Ok(schema) => Ok(schema),
        Err(schema_err) => Err(crate::web_err::ExecError::ParseError {
            content_type: crate::web_err::ContentType::SchemaJSON,
            file_name: PathBuf::from("in-memory-schema"),
            error: Box::new(schema_err),
        }),
    }
}
...
impl PolicySer {
    fn new(policy_id: &PolicyID, policy_set: &PolicySet) -> Self {
        let policy_str = policy_set.to_string();
        PolicySer {
