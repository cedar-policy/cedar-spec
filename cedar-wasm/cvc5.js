var Module = (() => {
    ...
    var _scriptDir = window.location.origin + '/';
    ...
    return function (Module) {
        ...
        function getBinaryPromise() {
            if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER)) {
                if (typeof fetch == 'function') {
                    console.log('Attempting to fetch WASM file from:', wasmBinaryFile);
                    return fetch(wasmBinaryFile, { credentials: 'same-origin' })
                        .then(function (response) {
                            console.log('WASM fetch response status:', response.status, response.statusText);
                            if (!response['ok']) {
                                throw "failed to load wasm binary file at '" + wasmBinaryFile + "' - Status: " + response.status;
                            }
                            return response['arrayBuffer']();
                        })
                        .then(function(arrayBuffer) {
                            console.log('WASM file loaded successfully, size:', arrayBuffer.byteLength, 'bytes');
                            return arrayBuffer;
                        })
        ...
        function createWasm() {
            ...
            // Emscripten imports memory by default, so provide it
            if (!wasmMemory) {
                wasmMemory = new WebAssembly.Memory({ initial: 1024, maximum: 65536 });
                updateGlobalBufferAndViews(wasmMemory.buffer);
                asmLibraryArg.memory = wasmMemory;
                asmLibraryArg.__memory_base = 1024;
                asmLibraryArg.emscripten_resize_heap = function(requestedSize) {
                    ...
                    while (newSize < requestedSize) {
                        if (newSize <= 536870912) {
                            ...
                            newSize = Math.min(newSize + 536870912, requestedSize + 134217728);
                        }
                    }
                    var replacement = new WebAssembly.Memory({initial: newSize / PAGE_MULTIPLE, maximum: 65536});
                    if (!replacement || replacement.buffer.byteLength != newSize) return false;
                    try {
                        new Uint8Array(replacement.buffer).set(new Uint8Array(wasmMemory.buffer));
                    } catch(e) {
                        return false;
                    }
                    wasmMemory = replacement;
                    asmLibraryArg.memory = wasmMemory;
                    updateGlobalBufferAndViews(wasmMemory.buffer);
                    return true;
                };
            ...
            function receiveInstance(instance, module) {
                ...
                Module['asm'] = exports;
                // Memory is imported, so it's already set up
            ...
            function instantiateArrayBuffer(receiver) {
                return getBinaryPromise()
                    ...
                    .then(function (instance) {
                        ...
                        console.log('Instance exports:', Object.keys(instance.instance.exports));
                        return instance;
                    })
                    .then(receiver, function (reason) {
                        console.error('WebAssembly instantiation failed:', reason);
        ...
        var FS = {
            ...
            nodePermissions: (node, perms) => {
                ...
                if (!node || !node.mode) {
                    return 2; // Permission denied for null nodes
                }
...
// Export Module as both default and named export for WebAssembly imports
